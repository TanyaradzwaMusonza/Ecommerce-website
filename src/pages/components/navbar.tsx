...
const Navbar: React.FC = () => {
  ...
  const handleSearchSubmit = () => {
    dispatchSearchEvent(searchQuery.trim());
  ...
  const handleAdminClick = () => {
    ...
    if (password === "Roshly") {
      window.location.href = "/dashboard/inventory-list";
    } else if (password) {
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          ...
          <div className="hidden md:flex items-center space-x-6">
            ...
            {/* Auth */}
            {!user ? (
              <Link href="/auth/login" className="px-4 py-2 bg-amber-700 text-white rounded-lg hover:bg-blue-300">Get Started</Link>
            ) : (
              <div className="relative">
                <button onClick={() => setAccountDropdownOpen(!isAccountDropdownOpen)} className="hover:text-blue-500 transition text-black flex items-center">
                  My Account
                </button>
          ...
          <div className="md:hidden flex items-center space-x-4">
            ...
            <button onClick={() => setMobileMenuOpen(!isMobileMenuOpen)} className="text-amber-600 hover:text-amber-800 transition">
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }} className="md:hidden bg-white/30 backdrop-blur-md shadow-lg px-4 py-4 space-y-4">
          <div className="flex flex-col space-y-2">
            ...
            />
            <button onClick={handleSearchSubmit} className="w-full bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700">Search</button>
          </div>
